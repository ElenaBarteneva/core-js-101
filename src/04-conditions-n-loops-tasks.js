 ...
 *
 */
function getIntervalString(point1, point2, isStartIncluded, isEndIncluded) {
  let a = point1;
  let b = point2;
  if (a > b) {
    [a, b] = [b, a];
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (open.includes(str[i])) {
      ...
      if (stack.length === 0 || stack[stack.length - 1] !== matchingBrackets[str[i]]) {
        return false;
      }
      stack.pop();
    }
...
function toNaryString(originalNum, n) {
  ...
  let result = '';
  const isNegative = originalNum < 0;
...
function getCommonDirectoryPath(pathes) {
  ...
  const result = commonPath.length > 0 ? `${commonPath.join('/')}/` : '/';
  return result;
}
